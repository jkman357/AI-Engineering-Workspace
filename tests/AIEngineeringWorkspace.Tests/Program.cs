...
Run("Version_SingleSource_IsRc15", TestVersionSource);
...
void TestVersionSource(){var doc=XDocument.Load(Path.Combine(FindRepositoryRoot(),"Directory.Build.props"));var values=doc.Descendants().ToDictionary(x=>x.Name.LocalName,x=>x.Value,StringComparer.OrdinalIgnoreCase);Assert(values["WorkspaceVersionLabel"]=="v0.0.6rc15","WorkspaceVersionLabel drift");Assert(values["Version"]=="0.0.6-rc15","package Version drift");Assert(values["FileVersion"]=="0.0.6.15","FileVersion drift");}
void TestWorkspaceProjectRoundTrip()
{
    var path=Path.Combine(Path.GetTempPath(),$"aiew-{Guid.NewGuid():N}.aew");
    try
    {
        var paneId=Guid.NewGuid();var project=new WorkspaceProjectDocument{ApplicationVersion="v0.0.6rc15",LayoutMode=WorkspaceLayoutMode.FreeLayout,ShowEndpointIds=true,Panes=new List<WorkspacePaneState>{new(){Kind=PaneKind.File,PaneId=paneId,DisplayIndex=2,X=123,Y=45,Width=456,Height=321,FilePath=@"C:\Example"},new(){Kind=PaneKind.Browser,PaneId=Guid.NewGuid(),DisplayIndex=1,X=600,Y=45,Width=720,Height=540}}};
...
void TestMultiPanePassGateDocumented(){var note=File.ReadAllText(Path.Combine(FindRepositoryRoot(),"docs","releases","v0.0.6rc15.md"));foreach(var token in new[]{"English/number","Zhuyin","maximize","restore","B1","B3","runtime"})Assert(note.Contains(token,StringComparison.OrdinalIgnoreCase),$"rc15 manual Browser focus gate missing token '{token}'");}
...
void TestImeInstrumentation()
{
    ...
    Assert(!coordinator.Contains("ActivateKeyboardLayout",StringComparison.Ordinal)&&!coordinator.Contains("PostMessage",StringComparison.Ordinal),"rc15 must retain rc14 read-only IME diagnostics and must not force/synchronize input layout");
}
