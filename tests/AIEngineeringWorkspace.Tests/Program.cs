...
Run("Version_SingleSource_IsRc13", TestVersionSource);
...
void TestVersionSource(){var doc=XDocument.Load(Path.Combine(FindRepositoryRoot(),"Directory.Build.props"));var values=doc.Descendants().ToDictionary(x=>x.Name.LocalName,x=>x.Value,StringComparer.OrdinalIgnoreCase);Assert(values["WorkspaceVersionLabel"]=="v0.0.6rc13","WorkspaceVersionLabel drift");Assert(values["Version"]=="0.0.6-rc13","package Version drift");Assert(values["FileVersion"]=="0.0.6.13","FileVersion drift");}
void TestWorkspaceProjectRoundTrip()
{
    var path=Path.Combine(Path.GetTempPath(),$"aiew-{Guid.NewGuid():N}.aew");
    try
    {
        var paneId=Guid.NewGuid();var project=new WorkspaceProjectDocument{ApplicationVersion="v0.0.6rc13",LayoutMode=WorkspaceLayoutMode.FreeLayout,ShowEndpointIds=true,Panes=new List<WorkspacePaneState>{new(){Kind=PaneKind.File,PaneId=paneId,DisplayIndex=2,X=123,Y=45,Width=456,Height=321,FilePath=@"C:\Example"},new(){Kind=PaneKind.Browser,PaneId=Guid.NewGuid(),DisplayIndex=1,X=600,Y=45,Width=720,Height=540}}};
...
void TestMultiPanePassGateDocumented(){var note=File.ReadAllText(Path.Combine(FindRepositoryRoot(),"docs","releases","v0.0.6rc13.md"));foreach(var token in new[]{"B1","111","B2","222","B3","333","B4","444","aaa","bbb","ccc","without pressing Focus"})Assert(note.Contains(token,StringComparison.OrdinalIgnoreCase),$"manual multi-pane PASS gate missing token '{token}'");}
